...
class CorradeConan(ConanFile):
    ...
    exports = ["COPYING"]
    exports_sources = ["CMakeLists.txt", "src/*", "package/conan/*", "modules/*", "toolchains/*"]
    generators = "cmake"
    ...
    def requirements(self):
        if self.settings.os == "Emscripten":
            self.requires("emsdk_installer/1.38.29@bincrafters/stable")
            self.requires("ninja_installer/1.8.2@bincrafters/stable")
    ...
    def package(self):
        ...
        if self.settings.build_type == "Debug":
            for lib in ALL_LIBS:
              old_name = os.path.join(self.package_folder, "lib", lib + "-d.bc")
              new_name = os.path.join(self.package_folder, "lib", lib + ".a")
    ...
    def package_info(self):
        ...
        self.cpp_info.libs = sort_libs(correct_order=ALL_LIBS, libs=builtLibs, lib_suffix="", reverse_result=True)
